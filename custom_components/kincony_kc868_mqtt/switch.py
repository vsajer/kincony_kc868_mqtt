"""Switches for KinCony KC868 MQTT Controller."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    for channel, cfg in mapping.get("outputs", {}).items():
        if cfg.get("enabled") and cfg.get("type") == "switch":
            entities.append(KinConySwitch(coordinator, channel, cfg))
    async_add_entities(entities)


class KinConySwitch(KinConyEntity, SwitchEntity):
    """KinCony output as switch."""
